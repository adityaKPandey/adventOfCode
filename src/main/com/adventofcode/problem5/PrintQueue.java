...
public class PrintQueue {
  ...
  public static void main(String[] args) {
    ...
    //int sum = getSumOfValidLists(adjacencyList, inputLists);
    //System.out.println("Sum = " + sum);

    int sum = correctInvalidListsAndGetSum(adjacencyList, inputLists);
    System.out.println("Sum = " + sum);

  }

  private static int getSumOfValidLists(Map<Integer, Set<Integer>> adjacencyList,
  ...
  private static int correctInvalidListsAndGetSum(Map<Integer, Set<Integer>> adjacencyList,
      List<List<Integer>> inputLists) {
    ...
    for(int i = 0 ; i < inputLists.size() ; i++){
         ...
         boolean check = true, invalid = false;
         ...
         while(check){
           ...
           for(int j = 0; j < array.length - 1; j++){
             for(int k = j+1; k < array.length; k++){
               if (adjacencyList.get(array[j]).contains(array[k])
                   && !adjacencyList.get(array[k]).contains(array[j])) {
                continue;
               }else{
                 System.out.println("invalid list : " + Arrays.toString(array));
                 invalid = true;
                 swaps++;
         ...
         System.out.println("valid list : " + Arrays.toString(array));
         if(invalid)
  ...
  private static Set<Integer> createAdjacencyListAndStoreInput(List<String> lines,
      Map<Integer, Set<Integer>> adjacencyList, List<List<Integer>> input) {
